...
# Then log them in to capture the JWT in a per-user cookie jar.
...
Record "VAL-2.8.4 Unknown email returns 404" `
  (((Call POST "/api/boards/$B1/members" $S1 @{ email = $Ghost }).Status) -eq 404)

Record "VAL-2.8.5 Inviting the owner rejected (400)" `
  (((Call POST "/api/boards/$B1/members" $S1 @{ userId = $U1Id }).Status) -eq 400)

Record "VAL-2.8.3 Empty invite body rejected (400)" `
  (((Call POST "/api/boards/$B1/members" $S1 @{}).Status) -eq 400)

Record "VAL-2.8.3 Both userId+email rejected (400)" `
  (((Call POST "/api/boards/$B1/members" $S1 @{ userId = $U2Id; email = $U2 }).Status) -eq 400)

Record "VAL-2.8.6 Duplicate pending invite rejected (409)" `
  (((Call POST "/api/boards/$B1/members" $S1 @{ email = $U2 }).Status) -eq 409)

# ---------------------------------------------------------------------------
# Step 4 - u2 lists pending invitations, sees $I1
# ---------------------------------------------------------------------------
$list = Call GET "/api/board-invitations" $S2
...
$list3 = Call GET "/api/board-invitations" $S3
...
Record "VAL-2.11.3 Double-accept rejected (409)" `
  (((Call POST "/api/board-invitations/$I1/accept" $S2).Status) -eq 409)

# ---------------------------------------------------------------------------
# Step 6 - u2 sees the board with role MEMBER; u3 gets 403
# ---------------------------------------------------------------------------
$lb2 = Call GET "/api/boards" $S2
...
Record "VAL-2.5.3 Missing board -> 404" `
  (((Call GET "/api/boards/00000000-0000-0000-0000-000000000000" $S1).Status) -eq 404)

# VAL-2.5.4 - non-UUID id -> 400.
Record "VAL-2.5.4 Non-UUID id -> 400" `
  (((Call GET "/api/boards/not-a-uuid" $S1).Status) -eq 400)

# ---------------------------------------------------------------------------
# Step 7-8 - u1 renames the board; u2 cannot
# ---------------------------------------------------------------------------
$ren = Call PATCH "/api/boards/$B1" $S1 @{ title = "Demo v2" }
...
Record "VAL-2.6.2 Member cannot rename (403)" `
  (((Call PATCH "/api/boards/$B1" $S2 @{ title = "Hax" }).Status) -eq 403)

# ---------------------------------------------------------------------------
# Step 9 - u1 lists members (OWNER first, then MEMBER)
# ---------------------------------------------------------------------------
$lm = Call GET "/api/boards/$B1/members" $S1
...
$inv3 = Call POST "/api/boards/$B1/members" $S1 @{ email = $U2 }
$I3 = $inv3.Body.id
Record "VAL-2.11.4 u3 cannot accept someone else's invite (403)" `
  (((Call POST "/api/board-invitations/$I3/accept" $S3).Status) -eq 403)
Record "u3 cannot decline someone else's invite (403)" `
  (((Call POST "/api/board-invitations/$I3/decline" $S3).Status) -eq 403)

# VAL-2.11.5 - accept of soft-deleted board -> 404.
$inv4 = Call POST "/api/boards/$B1/members" $S1 @{ email = $U2 }
$I4 = $inv4.Body.id
$del = Call DELETE "/api/boards/$B1" $S1
...
Record "VAL-2.11.5 Accept on soft-deleted board -> 404" `
  (((Call POST "/api/board-invitations/$I4/accept" $S2).Status) -eq 404)

# VAL-2.7.4 - deleting again -> 404.
Record "VAL-2.7.4 Second delete returns 404" `
  (((Call DELETE "/api/boards/$B1" $S1).Status) -eq 404)

# VAL-2.4.3 - soft-deleted board excluded from list.
$listAfterDel = Call GET "/api/boards" $S1
...
$cb2 = Call POST "/api/boards" $S1 @{ title = "SoftDelete" }
$B2 = $cb2.Body.id
$del2 = Call DELETE "/api/boards/$B2" $S1
Record "Soft-delete B2 (204)" ($del2.Status -eq 204)
Record "VAL-2.6.4 PATCH on soft-deleted board -> 404" `
  (((Call PATCH "/api/boards/$B2" $S1 @{ title = "Nope" }).Status) -eq 404)
