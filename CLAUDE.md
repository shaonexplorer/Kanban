...
features/
  ...
  board/                      # BoardView.tsx (tier-based), Column.tsx, types.ts, api.ts, useBoardQuery.ts + mutation hooks (move/rename/delete task+col, create task+col, invite/remove member, update board, create board, delete board)
...
- **`BoardHeader` board-settings menu:** an owner-only `more_horiz` icon between the invitations bell and the user avatar opens a `<div role="menu">` with **Rename** + **Delete**. The trigger is rendered only when `useAuth().userId === board.ownerId`, so non-owners see only bell + avatar. Rename swaps the title `<span>` for an inline `<input>` (Enter/blur commits, Esc reverts, max 100 chars; same contract as `ColumnShell.tsx:282-297`). The form state (`renamingBoard` + `renameDraft`) lives in `BoardView` so the input and the mutation share the same draft. Delete is the two-step "Click to confirm" pattern with 3s auto-disarm — same as `ColumnShell`, `TaskModal`, `InvitationsInbox`. On success the user is `router.push("/")`-ed and the cached `["boards"]` list (used by the Sidebar and the home page) is invalidated by `useDeleteBoardMutation`. The menu is always visible on every tier (compact/tablet/desktop) because the header is always mounted — no more hover-reveal guessing game, no more hiding behind a collapsed sidebar.
...
No `jest`/`vitest` yet — Steps 11/12 planned. Validation exercised by three PowerShell e2e scripts (`cd server` first; `npm run dev` on :4000). Phase 5 Step 10 (schema + new endpoints) is complete and all three pass (48 + 59 + 45 = 152 assertions total).
...
Phase 5 **Step 10** (schema widening + new endpoints) is **done**: `Task` fields `starred`/`priority`/`dueDate`/`storyPoints`/`labels` + `assignees` relation; `Board` fields `linkSharing`/`projectKey`/`colorIdentity`/`template`; `BoardUser.role` widened to `BoardRole` enum; new models `TaskSubtask`/`TaskComment`/`TaskAssignee`. Step 5 keeps the auth cookie's `secure` flag environment-gated so local e2e tests over HTTP keep working.
