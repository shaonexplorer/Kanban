...
Mini Kanban Board/
├── client/
│   └── kanban-board-client/    # Next.js 16 + React 19 + TypeScript + Tailwind CSS v4 — Phase 4 frontend (board view with dnd-kit + TanStack Query + dev-only Quick Register) complete; Phase 5 polish pending
├── server/                      # Backend (Node.js + Express + TypeScript + Prisma) — Phase 1, 2 & 3 complete; Phase 4 in progress (Steps 1–5 done: schema migration to position String + lexoPosition helper + smoke script + move endpoints for both tasks and columns + frontend board view with drag-and-drop; Steps 6–7 pending: manual verification + Validation.md update)
...
│   └── Phase04/               # Phase 4 specs: Plan.md, Requirements.md, Validation.md — Ordering & Task Movement (Steps 1–5 done: position Int → String migration on Column + Task + lexoPosition helper + smoke script + move endpoints for both tasks and columns + Next.js board view with @dnd-kit + TanStack Query; Steps 6–7 pending: end-to-end manual verification + Validation.md update)
...
- **Frontend:** Next.js 16 (App Router), React 19, TypeScript, Tailwind CSS v4, `@dnd-kit/core` + `@dnd-kit/sortable` + `@dnd-kit/utilities` (drag-and-drop), `@tanstack/react-query` (server state) — Phase 4 board view complete; auth is a dev-only `localStorage` JWT (Quick Register button on the home page); real auth UI is Phase 5
...
│   ├── page.tsx                  # Landing — dev-only Quick Register + paste-token helpers
│   ├── globals.css               # Tailwind v4 entry, light/dark CSS vars
│   └── boards/[id]/page.tsx      # Async server component, awaits Next.js 16 params
├── features/
│   ├── auth/
│   │   ├── AuthContext.tsx       # useSyncExternalStore over localStorage; Quick Register helper
...
│       └── TaskCard.tsx          # Sortable task card (useSortable)
...
- Auth: `AuthContext` exposes `{ token, setToken, clearToken, registerWithEmail }`. The home page's "Quick register (dev only)" button calls `registerWithEmail` and is explicitly labeled as removed in Phase 5. A real login / register UI is Phase 5.
