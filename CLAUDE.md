...
├── server/                      # Backend (Node.js + Express + TypeScript + Prisma) — Phase 1 & 2 complete; Phase 3 in progress (Steps 1–4 done)
...
The backend lives at `server/` and is organized as a **Modular MVC** layout on native ES Modules. **Phase 1 (Foundation) and Phase 2 (Boards & Access Control) are complete.** **Phase 3 (Columns & Tasks) is in progress — Steps 1, 2, 3 & 4 are done:** Step 1 (schema evolution) adds `onDelete: Cascade` from `Task → Column` so `DELETE /api/columns/:id` cleans up its tasks atomically; `Column → Board` and `Board → BoardUser` remain `RESTRICT` (boards still soft-delete via `deletedAt`). Step 2 extends the access-control layer with `loadColumn` and `loadTask` middlewares in `src/common/middleware/access-control.middleware.ts` — they fetch the column (with its board) or the task (with its column and that column's board) in a single Prisma query, treat missing or soft-deleted parent as 404, and expose the resolved board on `req.board` so the existing `requireBoardAccess` / `requireBoardOwner` middlewares chain behind them unchanged. Step 3 implements the `columns` module (CRUD + intra-board reorder) in `src/modules/columns/`, mounted on `/api` so a single router owns both `/api/boards/:boardId/columns` and `/api/columns/:id`. New columns are appended at `position = (max existing + 1)` (gaps allowed; Phase 4 will consume them); reorder takes the full ordered list of column ids and re-keys positions 0..N-1 atomically inside a `prisma.$transaction`. Step 4 implements the `tasks` module (CRUD; no reorder — that's Phase 4) in `src/modules/tasks/`, mounted on `/api` so a single router owns both `/api/columns/:columnId/tasks` and `/api/tasks/:id`. New tasks are appended at `position = (max existing + 1)` within their column. The Prisma schema includes `Board.deletedAt`, `BoardUser.joinedAt`, and a `BoardInvitation` model (with `BoardInvitationStatus` enum). The `boards` module (CRUD + member management + owner-driven invitations) is implemented in `src/modules/boards/`, the `board-invitations` module (list / accept / decline invitations addressed to the caller) is implemented in `src/modules/board-invitations/`, the `columns` module is implemented in `src/modules/columns/`, and the `tasks` module is implemented in `src/modules/tasks/`. `GET /api/boards/:id` is also populated in Step 5: `boards.service.ts` `getBoardById` now returns the nested shape with `columns[].tasks[]` ordered by `position` asc (per REQ-3.14.1–3.14.3) instead of the Phase 2 `columns: []` placeholder. Phase 3 ships no frontend work and no task reorder / cross-column move — both are reserved for Phase 4.
...
- Columns: `GET /api/boards/:boardId/columns` (list, `position` asc, 200), `POST /api/boards/:boardId/columns` (create, 201 with `{ id, title, boardId, position }`; new columns append at `position = (max existing + 1)`), `GET /api/columns/:id` (200), `PATCH /api/columns/:id` (rename, 200), `DELETE /api/columns/:id` (hard-delete, 204; cascading to tasks per `onDelete: Cascade` on `Task.column`), and `PATCH /api/boards/:boardId/columns/reorder` (atomic `prisma.$transaction` that re-keys positions 0..N-1 from the full ordered `columnIds` array; 400 when the set is not identical to the board's current column ids, 200 with the new ordering otherwise). All routes require `requireAuth`; board-scoped routes chain `validate(BoardScopedColumnParamSchema, "params") → loadBoard("params", "boardId") → requireBoardAccess` and column-scoped routes chain `validate(ColumnIdParamSchema, "params") → loadColumn → requireBoardAccess`. Phase 3 reuses `requireBoardAccess` for ALL column mutations (both owners and accepted members can author content); `position` is changed only via the reorder endpoint, not via `PATCH /api/columns/:id`.
- Tasks: `GET /api/columns/:columnId/tasks` (list, `position` asc, 200; empty array for an empty column), `POST /api/columns/:columnId/tasks` (create, 201 with `{ id, title, description, columnId, position, createdAt }`; new tasks append at `position = (max existing + 1)` within the column; body is `{ title: 1–200 chars, description?: ≤ 2000 chars }`), `GET /api/tasks/:id` (200, full task shape), `PATCH /api/tasks/:id` (200, partial of `{ title?, description? }` with `.refine()` enforcing at-least-one-field), `DELETE /api/tasks/:id` (hard-delete, 204). All routes require `requireAuth`; column-scoped routes chain `validate(ColumnScopedTaskParamSchema, "params") → loadColumn("params", "columnId") → requireBoardAccess` and task-scoped routes chain `validate(TaskIdParamSchema, "params") → loadTask → requireBoardAccess`. Phase 3 reuses `requireBoardAccess` for ALL task mutations; `position` and `columnId` are immutable on `PATCH /api/tasks/:id` — task reorder and cross-column move are reserved for Phase 4.
...
│   │   ├── columns/                 # columns.controller, columns.service, columns.validation, columns.routes, index — CRUD + intra-board reorder (Phase 3)
│   │   ├── tasks/                   # tasks.controller, tasks.service, tasks.validation, tasks.routes, index — CRUD (Phase 3; reorder + cross-column move deferred to Phase 4)
